...
class LiveModelSetupTests(unittest.TestCase):
    ...
    def test_workflow_keeps_cleanup_and_failure_export_independent(self):
        ...
        self.assertIn('test_live_model.py" --output',text)
        self.assertNotIn('--startup-only',text)
        self.assertIn('timeout --signal=TERM --kill-after=10s 540s',text)
        self.assertIn('    timeout-minutes: 15',text)
        self.assertIn('    runs-on: ubuntu-24.04',text)
        self.assertIn("github.ref == 'refs/heads/test/isolated-local-model'",text)
